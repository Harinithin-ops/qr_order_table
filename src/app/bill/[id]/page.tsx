...
export default function BillPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><Activity className="animate-spin text-gray-500" /></div>;
  if (!bill) return <div className="text-center p-10 font-bold text-xl">Bill not found</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 flex flex-col md:flex-row gap-8 items-start justify-center">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden self-center md:self-start">
          ...
          <div className="space-y-6 mb-8">
            ...
            {isAwaiting && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
                ...
                <button
                  ...
                  className="w-full bg-amber-600 text-white font-bold py-3 rounded-lg hover:bg-amber-700 transition disabled:opacity-50 flex items-center justify-center gap-2"
                >
            ...
            {!isPaid ? (
              <div className="grid grid-cols-2 gap-3">
                <button disabled={processing} onClick={() => handleMarkPaid('CASH')} className="border border-gray-300 rounded-lg p-3 font-semibold hover:border-gray-900 hover:bg-gray-50 transition flex justify-center">Cash</button>
                <button disabled={processing} onClick={() => handleMarkPaid('UPI')} className="border border-gray-300 rounded-lg p-3 font-semibold hover:border-gray-900 hover:bg-gray-50 transition flex justify-center">UPI (manual)</button>
          ...
          <div className="border-t border-gray-100 pt-6 space-y-3">
            {isClient && (
              <PDFDownloadLink 
                ...
                className="w-full bg-gray-900 text-white font-medium py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-black transition"
              >
                {({ loading }) => loading ? 'Generating PDF...' : <><FileDown size={18}/> Download PDF Receipt</>}
              </PDFDownloadLink>
            )}
            
            <Link href="/dashboard" className="w-full border border-gray-200 text-gray-700 font-medium py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-gray-50 transition">
